...
import { X, Cloud, CheckCircle, AlertTriangle, RotateCw, Play, Square, Filter, AlertCircle } from 'lucide-react';
...
export function EcsClusterModal({ clusterName, onClose }: EcsClusterModalProps) {
  ...
  const filteredServices = services.filter(service => {
    ...
    if (filterType === 'ecs-ec2') {
      ...
      if (statusFilter === 'running') return (service.type === 'ecs-ec2' || service.launchType === 'EC2') && service.status === 'running';
      if (statusFilter === 'stopped') return (service.type === 'ecs-ec2' || service.launchType === 'EC2') && service.status === 'stopped';
    ...
    if (filterType === 'ecs-fargate') {
      ...
      if (statusFilter === 'running') return service.type === 'ecs' && service.launchType === 'FARGATE' && service.status === 'running';
      if (statusFilter === 'stopped') return service.type === 'ecs' && service.launchType === 'FARGATE' && service.status === 'stopped';
  ...
  const handleDesiredCountChange = async (service: AWSService) => {
    try {
      ...
      const allServices = useServicesStore.getState().services;
      const clusterServices = allServices.filter(
        s => s.type === 'ecs' && s.cluster === clusterName
      );
      setServices(clusterServices);
    } catch (err) {
