...
export function AlbRulesModal({ albArn, albName, onClose }: AlbRulesModalProps) {
  ...
  const loadListeners = async () => {
    ...
    try {
      ...
      const response = await clients.elb.send(new DescribeListenersCommand({
  ...
  const loadRules = async () => {
    ...
    try {
      const clients = createAWSClients(selectedRegion);
      const response = await clients.elb.send(new DescribeRulesCommand({
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="p-4 overflow-y-auto max-h-[calc(90vh-8rem)]">
          ...
          {isLoading ? (
            ...
            <>
              ...
              {rules.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  No rules found for this listener.
                </div>
              ) : (
                              ...
                              <div className="space-y-2">
                                {rule.Conditions?.map((condition, index) => (
                                  <div key={index} className="flex items-center space-x-2">
                                    ...
                                    <input
                                      type="text"
                                      value={editedValues[rule.RuleArn || '']?.[`condition_${index}`] || condition.Values?.join(', ') || ''}
                                      onChange={(e) => handleValueChange(rule.RuleArn || '', `condition_${index}`, e.target.value)}
                              ...
                              <div>
                                {rule.Conditions?.map((condition, index) => (
                                  <div key={index} className="text-xs">
                                    <span className="font-medium">{condition.Field}:</span> {condition.Values?.join(', ')}
                                  </div>
                                ))}
                              </div>
                            )}
                                    ...
                                    {action.Type === 'fixed-response' ? (
                                      <>
                                        <div className="flex items-center space-x-2">
                                          ...
                                          <input
                                            type="text"
                                            value={editedValues[rule.RuleArn || '']?.[`action_${index}_content_type`] || action.FixedResponseConfig?.ContentType || ''}
                                            onChange={(e) => handleValueChange(rule.RuleArn || '', `action_${index}_content_type`, e.target.value)}
                                            className="border border-gray-300 rounded px-2 py-1 text-sm w-full"
                                        ...
                                        <div className="flex items-center space-x-2">
                                          ...
                                          <input
                                            type="text"
                                            value={editedValues[rule.RuleArn || '']?.[`action_${index}_status_code`] || action.FixedResponseConfig?.StatusCode || ''}
                                            onChange={(e) => handleValueChange(rule.RuleArn || '', `action_${index}_status_code`, e.target.value)}
                                            className="border border-gray-300 rounded px-2 py-1 text-sm w-full"
                                            placeholder="200"
                                          />
                                        </div>
                                        <div className="flex items-center space-x-2">
                                          ...
                                          <textarea
                                            value={editedValues[rule.RuleArn || '']?.[`action_${index}_message_body`] || action.FixedResponseConfig?.MessageBody || ''}
                                            onChange={(e) => handleValueChange(rule.RuleArn || '', `action_${index}_message_body`, e.target.value)}
                                            className="border border-gray-300 rounded px-2 py-1 text-sm w-full"
                                            rows={3}
                                      ...
                                      <div className="flex items-center space-x-2">
                                        ...
                                        <input
                                          type="text"
                                          value={editedValues[rule.RuleArn || '']?.[`action_${index}_target_group`] || action.TargetGroupArn || ''}
                                          onChange={(e) => handleValueChange(rule.RuleArn || '', `action_${index}_target_group`, e.target.value)}
                                          className="border border-gray-300 rounded px-2 py-1 text-sm w-full"
                                        />
                                      </div>
                                    )}
            ...
            <div className="text-center py-8 text-gray-500">
              No listeners found for this ALB.
            </div>
          )}
